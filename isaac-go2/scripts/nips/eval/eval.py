...
"""
python -m src.scripts.ddpg.eval --logdir=logs/train/ddpg_trot/demo --num_envs=1 --use_gpu=True --enable_phy_teacher=True
"""
...
flags.DEFINE_bool("enable_phy_teacher", False, "whether to enable the PHY-Teacher.")
...
def main(argv):
    ...
    with config.unlocked():
        ...
        # config.environment.action_lb = np.array([-.1, -.1, -.1, -.1, -.1, -.1])
        # config.environment.action_ub = np.array([.1, .1, .1, .1, .1, .1])
    ...
    if FLAGS.enable_phy_teacher:
        with config.unlocked():
            config.environment.phy_teacher.learning_space = np.array(
                [0.04, 0.28, 0.28, np.inf, 0.33, np.inf, np.inf, np.inf, np.inf, 1.])*0.001
            config.environment.phy_teacher.enable = True
            config.environment.phy_teacher.chi = 0.15
            config.environment.phy_teacher.tau = 20
    ...
    with torch.inference_mode():
        while True:
            ...
            state, _, nominal_action, action_type, reward, done, info = env.step(action)
    ...
    if FLAGS.use_real_robot or FLAGS.save_traj:
        ...
        os.makedirs(os.path.dirname(FLAGS.traj_dir), exist_ok=True)
