...
def main(argv):
    ...
    if FLAGS.enable_phy_teacher:
        with config.unlocked():
            config.environment.phy_teacher.learning_space = [0.04, 0.28, 0.28, np.inf, 0.33, np.inf, np.inf, np.inf,
                                                            np.inf, 1.]
            config.environment.phy_teacher.enable = True
            config.environment.phy_teacher.chi = 0.2
