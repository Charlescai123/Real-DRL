...
"""
python -m src.scripts.sac.eval --logdir=logs/train_sac --num_envs=1 --use_gpu=True --enable_phy_teacher=True
"""
...
def main(argv):
    ...
    with config.unlocked():
        ...
        config.environment.desired_vx = 0.35
    ...
    if FLAGS.enable_phy_teacher:
        with config.unlocked():
            config.environment.phy_teacher.learning_space = [0.04, 0.28, 0.28, np.inf, 0.33, np.inf, np.inf, np.inf, np.inf, 1.]
