...
from src.drl_student.agents.abstract.agent import Agent
from src.drl_student.env.vec_env import VecEnv
from src.drl_student.networks.network import Network
from src.drl_student.storage.storage import Dataset
from src.drl_student.utils.vec_env_utils import environment_dimensions
...
class AbstractActorCritic(Agent):
    ...
    def update(self, dataset: Dataset, action_type) -> Dict[str, Union[float, torch.Tensor]]:
        with torch.inference_mode():
            self.storage.append(self._process_dataset(dataset), action_type)
