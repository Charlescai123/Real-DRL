...
def check_trigger_type(trigger_type: int):
    """Check the Trigger type"""
    if trigger_type == 0:
        res = TriggerType.SELF
    elif trigger_type == 1:
        res = TriggerType.EVENT
    else:
        res = TriggerType.UNKNOWN
    return res


def check_safety(error_state: torch.Tensor, learning_space: torch.Tensor):
    """Check current safety condition w.r.t the system's error states"""
    is_activate = torch.abs(error_state[:, 2:]) >= learning_space
