...
from src.utils.utils import ActionMode, PlotMode
from src.logger.utils import check_dir
...
class FigPlotter:
    ...
    def plot_phase(self, state_list, action_mode_list, idx, plot_mode=PlotMode.POSITION):
        ...
        h3, = plt.plot(phases[-1][0], phases[-1][1], '-', color=[0, 0.4470, 0.7410], label="DRL-Student", markersize=2)
        h4, = plt.plot(phases[-1][0], phases[-1][1], 'r-', label="PHY-Teacher", markersize=2)
        ...
        #     file.write(f"episode {idx}: phy_teacher: {teacher_cnt}, drl_student: {student_cnt}\n")
        #     file.close()
    ...
    def legend_and_label(axes, x_ticks, th_ticks, f_ticks):
        ...
        axes[0, 0].add_line(mlines.Line2D([], [], color=[0, 0.4470, 0.7410], linestyle='-', label='DRL-Student'))
        axes[0, 0].add_line(mlines.Line2D([], [], color='red', linestyle='-', label='PHY-Teacher'))
        ...
        axes[0, 1].add_line(mlines.Line2D([], [], color=[0, 0.4470, 0.7410], linestyle='-', label='DRL-Student'))
        axes[0, 1].add_line(mlines.Line2D([], [], color='red', linestyle='-', label='PHY-Teacher'))
        ...
        axes[1, 0].add_line(mlines.Line2D([], [], color=[0, 0.4470, 0.7410], linestyle='-', label='DRL-Student'))
        axes[1, 0].add_line(mlines.Line2D([], [], color='red', linestyle='-', label='PHY-Teacher'))
        ...
        axes[1, 1].add_line(mlines.Line2D([], [], color=[0, 0.4470, 0.7410], linestyle='-', label='DRL-Student'))
        axes[1, 1].add_line(mlines.Line2D([], [], color='red', linestyle='-', label='PHY-Teacher'))
        legend_without_duplicate_labels(axes[1, 1])

        # Add label and title (force)
        axes[2, 0].set_yticks(f_ticks)
        axes[2, 0].set_ylabel("force (N)")
        axes[2, 0].add_line(mlines.Line2D([], [], color=[0, 0.4470, 0.7410], linestyle='-', label='DRL-Student'))
        axes[2, 0].add_line(mlines.Line2D([], [], color='red', linestyle='-', label='PHY-Teacher'))
        ...
        axes[2, 1].add_line(mlines.Line2D([], [], color=[0, 0.4470, 0.7410], linestyle='-', label='DRL-Student'))
        axes[2, 1].add_line(mlines.Line2D([], [], color='red', linestyle='-', label='PHY-Teacher'))
    ...
    def line_segment(axes, state1, state2, action1, action2, action_mode, energy1, energy2, i):
        if action_mode == ActionMode.STUDENT:
            # x
            axes[0, 0].plot([i, i + 1], [state1[0], state2[0]], '-', label='DRL-Student', color=[0, 0.4470, 0.7410])

            # x_dot
            axes[0, 1].plot([i, i + 1], [state1[1], state2[1]], '-', label='DRL-Student', color=[0, 0.4470, 0.7410])

            # theta
            axes[1, 0].plot([i, i + 1], [state1[2], state2[2]], '-', label='DRL-Student', color=[0, 0.4470, 0.7410])

            # theta_dot
            axes[1, 1].plot([i, i + 1], [state1[3], state2[3]], '-', label='DRL-Student', color=[0, 0.4470, 0.7410])

            # force/action
            axes[2, 0].plot([i, i + 1], [action1, action2], '-', label='DRL-Student', color=[0, 0.4470, 0.7410])

            # system energy
            axes[2, 1].plot([i, i + 1], [energy1, energy2], '-', label='DRL-Student', color=[0, 0.4470, 0.7410])

        elif action_mode == ActionMode.TEACHER:
            # x
            axes[0, 0].plot([i, i + 1], [state1[0], state2[0]], 'r-', label='PHY-Teacher')

            # x_dot
            axes[0, 1].plot([i, i + 1], [state1[1], state2[1]], 'r-', label='PHY-Teacher')

            # theta
            axes[1, 0].plot([i, i + 1], [state1[2], state2[2]], 'r-', label='PHY-Teacher')

            # theta_dot
            axes[1, 1].plot([i, i + 1], [state1[3], state2[3]], 'r-', label='PHY-Teacher')

            # force/action
            axes[2, 0].plot([i, i + 1], [action1, action2], 'r-', label='PHY-Teacher')

            # system energy
            axes[2, 1].plot([i, i + 1], [energy1, energy2], 'r-', label='PHY-Teacher')
    ...
    def plot_envelope(p_mat, epsilon):
        ...
        # PHY-Teacher switch envelope
        # if self.simplex_enable:
        #     plt.plot(tx_eps1, tx_eps2, 'k--', linewidth=0.8, label=r"$\partial\Omega_{PHY-Teacher}$")

        # DRL-Student switch envelope
        # plt.plot(tx_DRL-Student1, tx_DRL-Student2, 'b--', linewidth=0.8, label=r"$\partial\Omega_{DRL-Student}$")
