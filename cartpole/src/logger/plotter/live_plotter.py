...
class LivePlotter:
    ...
    def line_segment(axes, action_mode, i):
        ...
        if action_mode == ActionMode.STUDENT:
            # x
            line, = axes[0, 0].plot([i, i + 1], [y1, y2], '-', label='DRL-Student', color=[0, 0.4470, 0.7410])
            line.set_ydata([y1, y2])
            # axes[0, 0].set_xlim([0, 50])
            lines.append(line)

            # x_dot
            line, = axes[0, 1].plot([i, i + 1], [y1, y2], '-', label='DRL-Student', color=[0, 0.4470, 0.7410])
            lines.append(line)

            # theta
            line, = axes[1, 0].plot([i, i + 1], [y1, y2], '-', label='DRL-Student', color=[0, 0.4470, 0.7410])
            lines.append(line)

            # theta_dot
            line, = axes[1, 1].plot([i, i + 1], [y1, y2], '-', label='DRL-Student', color=[0, 0.4470, 0.7410])
            lines.append(line)

            # force/action
            line, = axes[2, 0].plot([i, i + 1], [y1, y2], '-', label='DRL-Student', color=[0, 0.4470, 0.7410])
            lines.append(line)

            # system energy
            line, = axes[2, 1].plot([i, i + 1], [y1, y2], '-', label='DRL-Student', color=[0, 0.4470, 0.7410])
            lines.append(line)
            ...
            # x
            line, = axes[0, 0].plot([i, i + 1], [y1, y2], 'r-', label='PHY-Teacher')
            lines.append(line)

            # x_dot
            line, = axes[0, 1].plot([i, i + 1], [y1, y2], 'r-', label='PHY-Teacher')
            lines.append(line)

            # theta
            line, = axes[1, 0].plot([i, i + 1], [y1, y2], 'r-', label='PHY-Teacher')
            lines.append(line)

            # theta_dot
            line, = axes[1, 1].plot([i, i + 1], [y1, y2], 'r-', label='PHY-Teacher')
            lines.append(line)

            # force/action
            line, = axes[2, 0].plot([i, i + 1], [y1, y2], 'r-', label='PHY-Teacher')
            lines.append(line)

            # system energy
            line, = axes[2, 1].plot([i, i + 1], [y1, y2], 'r-', label='PHY-Teacher')
            lines.append(line)
